                ...
                json={
                    ...
                    "messages": [{
                        ...
                        "content": [
                            ...
                            {
                                "type": "text",
                                "text": """Analizá esta captura de comprobante de pago bancario o transferencia.
Extraé SOLO estos datos en formato JSON puro, sin texto adicional ni backticks:
{"monto": numero_sin_simbolos, "beneficiario": "nombre destinatario", "fecha": "DD/MM/YYYY", "concepto": "descripcion si aparece", "moneda": "PEN o USD"}
Si no podés leer algún dato, ponelo como null."""
        ...
        logger.error(f"Error en analizar_captura: {e}")
        return {"monto": None, "beneficiario": None, "fecha": None, "concepto": None, "moneda": "PEN"}
...
async def cmd_hoy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Error en cmd_hoy: {e}")
        await update.message.reply_text(f"❌ Error conectando con el Sheet: {str(e)}")
...
async def cmd_semana(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Error en cmd_semana: {e}")
        await update.message.reply_text(f"❌ Error conectando con el Sheet: {str(e)}")
...
async def cmd_mes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        total_general = total_fijos + total_vars
        porcentaje = ((pagado_fijos+pagado_vars)/total_general*100) if total_general > 0 else 0
        texto = (
            f"📊 *Resumen del mes*\n\n"
            f"💰 Total fijos: {format_sol(total_fijos)}\n"
            f"📦 Total variables: {format_sol(total_vars)}\n"
            f"━━━━━━━━━━━━━━\n"
            f"🧾 *Total mes: {format_sol(total_general)}*\n\n"
            f"✅ Pagado: {format_sol(pagado_fijos + pagado_vars)}\n"
            f"⏳ Pendiente: {format_sol(pendiente)}\n"
            f"📈 Ejecutado: {porcentaje:.1f}%"
        )
        await update.message.reply_text(texto, parse_mode="Markdown")
    except Exception as e:
        logger.error(f"Error en cmd_mes: {e}")
        await update.message.reply_text(f"❌ Error conectando con el Sheet: {str(e)}")
...
async def cmd_pendientes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Error en cmd_pendientes: {e}")
        await update.message.reply_text(f"❌ Error conectando con el Sheet: {str(e)}")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"No pude buscar match en sheet: {e}")
        keyboard = [
        ...
        await update.message.reply_text("❌ Ocurrió un error procesando la imagen. Intentá de nuevo.")
    ...
    if data == "confirmar_pago":
        ...
        if match:
            try:
                ...
                await query.edit_message_text(f"❌ Error al guardar en Sheet: {str(e)}")
        else:
